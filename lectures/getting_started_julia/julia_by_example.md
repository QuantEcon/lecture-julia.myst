...
```{warning}
If you did not do the `] instantiate` step or are running the notebooks in a separate location without the Project and Manifest files (e.g., colab) this will install the packages but will take a long time in the first run.
```
