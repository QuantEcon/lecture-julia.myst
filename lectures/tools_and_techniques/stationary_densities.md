...
We'll use a simple helper `lae_est(p, X, ygrid)` that evaluates the look-ahead estimator directly

* `p` is the stochastic kernel
* `X` is a vector of draws of the previous-period state
* `ygrid` is a vector of evaluation points

The helper reshapes inputs so that `p.(X, ygrid)` broadcasts over all combinations and returns the sample mean across draws.
